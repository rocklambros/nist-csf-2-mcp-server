/**
 * Get Assessment Questions Tool - Unit Tests
 */
...
describe('Get Assessment Questions Tool - Unit Tests', () => {
  ...
  describe('basic functionality', () => {
    it('should execute successfully with valid parameters', async () => {
      ...
      const result = await get_assessment_questions(params);
      testUtils.assertSuccessResponse(result);
    });

    it('should handle missing required parameters', async () => {
      const result = await get_assessment_questions({});
  ...
  const result = await get_assessment_questions({ id: 'test-123' });
  ...
  const result = await get_assessment_questions({ profile_id: 'test-123' });
  testUtils.assertErrorResponse(result);
    });
  });
});