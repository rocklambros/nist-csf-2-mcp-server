/**
 * Generate Compliance Report Tool - Unit Tests
 */

import { describe, it, expect, beforeEach, jest } from '@jest/globals';
import { generate_compliance_report } from '../../src/tools/generate_compliance_report.js';
import { getDatabase } from '../../db/database.js';
import { createMockDatabase, testUtils } from '../helpers/jest-setup.js';

const mockGetDatabase = getDatabase as jest.MockedFunction<typeof getDatabase>;

describe('Generate Compliance Report Tool - Unit Tests', () => {
  ...
  describe('basic functionality', () => {
    it('should execute successfully with valid parameters', async () => {
      ...
      const result = await generate_compliance_report(params);
      testUtils.assertSuccessResponse(result);
    });

    it('should handle missing required parameters', async () => {
      const result = await generate_compliance_report({});
  ...
  const result = await generate_compliance_report({ id: 'test-123' });
  ...
  const result = await generate_compliance_report({ profile_id: 'test-123' });
  testUtils.assertErrorResponse(result);
    });
  });
});