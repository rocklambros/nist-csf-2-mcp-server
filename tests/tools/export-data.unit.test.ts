/**
 * Export Data Tool - Unit Tests
 */

import { describe, it, expect, beforeEach, jest } from '@jest/globals';
import { export_data } from '../../src/tools/export_data.js';
...
describe('Export Data Tool - Unit Tests', () => {
  ...
  describe('basic functionality', () => {
    it('should execute successfully with valid parameters', async () => {
      ...
      const result = await export_data(params);
      testUtils.assertSuccessResponse(result);
    });

    it('should handle missing required parameters', async () => {
      const result = await export_data({});
  ...
  const result = await export_data({ id: 'test-123' });
  ...
  const result = await export_data({ profile_id: 'test-123' });
  testUtils.assertErrorResponse(result);
    });
  });
});